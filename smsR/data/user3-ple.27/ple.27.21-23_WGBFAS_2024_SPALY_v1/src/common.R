...
sdplot <- function (fit, barcol = NULL, marg = NULL, ylim = NULL, show.rel.w = FALSE, 
    ...){
    ...
    v <- cf
    v[] <- c(NA, sd)[cf + 2]
    res <- data.frame(fleet = fn[as.vector(row(v))], name = paste0(fn[as.vector(row(v))], 
        " age ", ages[as.vector(col(v))]), sd = as.vector(v))
    res <- res[complete.cases(res), ]
    o <- order(res$sd)
    res <- res[o, ]
    res$rel.w <- (1/res$sd^2)/max(1/res$sd^2)
    if (missing(barcol)) 
        barcol <- colors()[as.integer(as.factor(res$fleet)) * 
            10]
    if (missing(marg)) 
    ...
    else barplot(res$sd, names.arg = res$name, las = 2, col = barcol, 
        ylab = "SD", ylim = ylim)
    box()
}